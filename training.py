...
def get_data_from_yahoo(stock: str, out_file: Optional[str] = None, random_order=False):
    ...
    for index, d in stock_data.iterrows():
        in_data.append([d["Open"], d["Close"], d["Open"]-d["Close"]])
    ...
    with open(out_file, "w") as f:
        l = [list(scaler.data_min_), list(scaler.data_max_), training_data, training_answers, test_data, test_answers]
        json.dump(l, f, indent=2)

    return scaler, training_data, training_answers, test_data, test_answers


def train(training_data: torch.Tensor, training_answers: torch.Tensor, device: str, network: Type[torch.nn.Module], arg):
...
def run(model: Type[torch.nn.Module], scaler: MinMaxScaler, input_data: torch.Tensor, true_values: torch.Tensor):
    scaled_predicted = []
    ...
    for inp in input_data:
        ...
        output = model(inp)
        output = scaler.inverse_transform([[output[0].item(), 0, 0]])[0]
    ...
    plt.plot([0, max(predicted)], [0, max(predicted)])
    plt.show()
...
def main():
    ...
    if os.path.exists(f"data/{s}_data.txt"):
        ...
        scaler, training_data, training_answers, test_data, test_answers = get_data_from_yahoo(s, f"data/{s}_data.txt")
    ...
    model = train(training_data, training_answers, device, LSTM, (len(training_data[0]), len(training_data[0][0]), 3))
    test(model, test_data, test_answers)
    run(model, scaler, training_data, training_answers)
